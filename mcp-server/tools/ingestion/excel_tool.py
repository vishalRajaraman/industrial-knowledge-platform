...
async def ingest_excel(
    ...
    # ── ERP Row-Level Graph Ingestion ─────────────────────────────────────
    if doc_type == "maintenance":
        for sheet, df in processed_dfs.items():
            for _, row in df.iterrows():
                wo_id = str(row.get("work_order_id", "")).strip()
                if not wo_id or wo_id.lower() == 'nan':
                    continue
                    
                status = str(row.get("status", "")).strip()
                desc = str(row.get("description", "")).strip()
                
                # Create WorkOrder Node
                await neo4j_client.upsert_node(
                    wo_id, ["WorkOrder"], 
                    {"work_order_id": wo_id, "status": status, "description": desc}
                )
                await neo4j_client.upsert_edge(wo_id, doc_id, "BELONGS_TO")
                
                # Connect to Equipment
                eq_tag = str(row.get("equipment_tag", "")).strip().upper()
                if eq_tag and eq_tag.lower() != 'nan':
                    eq_name = str(row.get("equipment_name", "")).strip()
                    await neo4j_client.upsert_node(eq_tag, ["Equipment"], {"tag": eq_tag, "name": eq_name})
                    await neo4j_client.upsert_edge(wo_id, eq_tag, "INVOLVES")
                
                # Connect to Vendor
                vendor = str(row.get("vendor_name", "")).strip()
                if vendor and vendor.lower() != 'nan':
                    # Clean vendor ID if we want or just use name
                    await neo4j_client.upsert_node(vendor, ["Vendor"], {"name": vendor})
                    await neo4j_client.upsert_edge(wo_id, vendor, "ASSIGNED_TO")
...
def register(mcp: FastMCP):
    ...
    async def ingest_excel_tool(
        ...
        Args:
            ...
            upload_to_s3: Upload original file to S3 raw asset store.
        """
        return await ingest_excel(file_path, sheet_name, doc_type, upload_to_s3)
